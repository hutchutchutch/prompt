...
export const ModelCarousel: React.FC = () => {
  const { modelIdx, nextModel } = usePromptStore();
  ...
  useEffect(() => {
    ...
    const startInterval = () => {
      intervalRef.current = setInterval(() => {
        nextModel();
      }, 7000); // 7s cadence, different than the prompt carousel to avoid simultaneous changes
    };
  ...
  const handleCardClick = (index: number) => {
    const position = getPosition(index, modelIdx, models.length);
    if (position === 'prev' || position === 'next') {
      usePromptStore.getState().setModelIdx(index);
    }
  };
        ...
        <div className="relative h-[200px] w-full max-w-[250px] mx-auto" style={{ perspective: '1000px' }}>
          {models.map((model, index) => (
            <motion.div
              ...
              animate={getPosition(index, modelIdx, models.length)}
              whileHover={{ scale: getPosition(index, modelIdx, models.length) === 'active' ? 1.02 : undefined }}
              onClick={() => handleCardClick(index)}
              style={{ 
                transformStyle: 'preserve-3d',
                pointerEvents: getPosition(index, modelIdx, models.length) === 'inactive' ? 'none' : 'auto'
              }}
            >
              <Card className={`h-full shadow-card border border-border/50 dark:border-border/30 overflow-hidden ${
                getPosition(index, modelIdx, models.length) === 'active' ? 'animate-float' : ''
              }`}>
                <CardContent className="p-4 h-full">
                  <div className="mb-2">
                    <h3 className="text-lg font-semibold">{model.title}</h3>
                  </div>
                  <div className="text-sm text-muted-foreground prompt-font overflow-y-auto max-h-28 p-2 bg-muted/30 rounded">
                    {model.content}
                  </div>
                </CardContent>
              </Card>
